...
public class SelectorManager : MonoBehaviour
{
    ...
    private Vector2 vecStart;//클릭 시작점
    private Vector2 vecEnd;//클릭 마지막 이동점
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.Mouse0))//마우스 좌클릭 시작
        {
    ...
    {
        Vector2 vecCenter = (vecStart + vecEnd) * 0.5f;//기준점을 잡아준다
        rectTrs.position = vecCenter;

        float sizeX = Mathf.Abs(vecStart.x - vecEnd.x);//절댓값
        float sizeY = Mathf.Abs(vecStart.y - vecEnd.y);//절댓값
